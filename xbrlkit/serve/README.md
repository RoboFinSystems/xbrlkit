...
| `search_text`, `read_text` | regex search over the readable text — the whole primary document, or the tagged text blocks alone — and paging from an offset. A pattern that matches more than came back also says where all of the matches fall, by section; one that matches nothing counts its own words separately, so a phrase the filer words differently is a step rather than a dead end |
