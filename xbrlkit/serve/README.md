...
| `information_block` | the filing's blocks as families read from the filer's own role titles — each note with its policies, tables and details, each statement with its parenthetical, the cover page — as a list with counts, or one family's index by topic; the cheap call |
| `information_block_full` | one block read whole: rows in filing order with consolidated values, the same rows by the block's own axes, its calculation arcs footed per period, and its text blocks with offsets — the expensive call, after `information_block` |
