...
The ClawDog shape is the authored-report lane beside holon and TAVI: a JSON-LD
graph of report, entity, periods, units, concepts, facts, networks, and the
calculation equations implied by calculation networks. It keeps producer fact
ids and fact provenance first-class, because those are the anchors a
ledger-authored report needs before it becomes a portable reporting object.

This module defines version 1 of the ClawDog report JSON-LD document shape.
...
def _report_node(model: XbrlModel, report_id: str) -> dict[str, object]:
  ...
  _put(node, "lg:acceptanceDatetime", filing.acceptance_datetime)
  _put(node, "lg:isInlineXBRL", filing.is_inline_xbrl)
  ...
  _put(node, "lg:primaryDocument", filing.primary_document)
  _put(node, "lg:documentName", filing.document_name)
...
def _concept_node(model: XbrlModel, qname: str, gaps: GapReport) -> dict[str, object]:
  ...
  if concept.references:
    node["lg:references"] = [
      reference.model_dump(exclude_none=True) for reference in concept.references
    ]
  if qname not in _mentioned_concepts(model):
    gaps.notes.append(f"unmentioned concept: {qname}")
  return node
...
def _fact_node(fact: XbrlFact, gaps: GapReport) -> dict[str, object]:
  ...
  _put(node, "lg:rawValue", fact.raw_value)
  _put(node, "lg:sourceHash", fact.source_hash)
...
def _network_node(network: Network, index: int, gaps: GapReport) -> dict[str, object]:
  ...
  _put(node, "lg:roleId", network.role_id)
