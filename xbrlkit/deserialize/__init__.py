...
TAVI compiled model, a holon or the property graph's rows — a report that was
never an SEC filing, a ledger's own output, a filing someone else converted, a
filing sitting in a graph — turns it into the same ``XbrlModel`` and gets the
whole toolset over it, with no Arelle and no XBRL.
...
from .graph import GraphError, from_graph, from_graph_report, read_lbug
