...
from ..serialize.clawdog import CLAWDOG_VOCAB, DOCTYPE_CLAWDOG, STANDARD_LABEL
...
def _is_clawdog(document: Mapping[str, Any]) -> bool:
  ...
  return any(_has_type(_mapping(node), "Report") for node in graph) and _has_lg_vocab(
    document.get("@context")
  )


def _has_lg_vocab(context: Any) -> bool:
  for item in _list(context):
    if isinstance(item, Mapping) and item.get("lg") == CLAWDOG_VOCAB:
      return True
  return False
...
def _filing(
  ...
  return FilingMeta(
    ...
    cik=entity.cik,
    reporting_style=_text(_prop(report, "reportingStyle")),
    report_meta=_dict_or_none(_prop(report, "reportMeta")),
    form=_text(_prop(report, "form")),
    filing_date=_date(_prop(report, "filingDate")),
    report_date=_date(_prop(report, "reportDate")),
    acceptance_datetime=_text(_prop(report, "acceptanceDatetime")),
    is_inline_xbrl=_bool_or_none(_prop(report, "isInlineXBRL")),
    ...
    taxonomy_namespaces=_taxonomy_namespaces(report, concepts),
    primary_document=_text(_prop(report, "primaryDocument")),
    report_uri=_text(_prop(report, "reportURI")),
    extension_namespace=_text(_prop(report, "extensionNamespace")),
    document_name=_text(_prop(report, "documentName")),
    items=[str(item) for item in _list(_prop(report, "items"))],
  )


def _taxonomy_namespaces(
  report: Mapping[str, Any], concepts: Mapping[str, Concept]
) -> list[str]:
  namespaces = [
    str(ns) for ns in _list(_prop(report, "taxonomyNamespaces")) if isinstance(ns, str)
  ]
  if namespaces:
    return namespaces
  return sorted(
...
def _concepts(nodes: Sequence[Mapping[str, Any]]) -> dict[str, Concept]:
  ...
  for node in nodes:
    ...
    namespace = (
      _text(_prop(node, "namespace"))
      if _prop(node, "namespace") is not None
      else (prefix if local else "")
    )
    concepts[qname] = Concept(
      ...
      references=[
        Reference(
          value=_text(_mapping(reference).get("value")) or "",
          role=_text(_mapping(reference).get("role")),
        )
        for reference in _list(_prop(node, "references"))
  ...
  for node in nodes:
    ...
    facts.append(
      XbrlFact(
        ...
        raw_value=_text(_prop(node, "rawValue")),
        source_hash=_text(_prop(node, "sourceHash")),
  ...
  for node in nodes:
    ...
    networks.append(
      Network(
        ...
        kind=kind,
        role_id=_text(_prop(node, "roleId")),
        arcs=[
          Arc(
