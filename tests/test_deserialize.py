...
def test_clawdog_round_trips_the_whole_model(model: XbrlModel) -> None:
  """The reviewer gate: model -> ClawDog -> model, compared field by field."""
  subject = model.model_copy(deep=True)
  subject.filing.reporting_style = None
  subject.filing.form = None
  subject.filing.acceptance_datetime = "2025-02-14T12:34:56Z"
  subject.filing.primary_document = "acme-20241231.htm"
  subject.filing.document_name = "acme-20241231.htm"
  subject.concepts["us-gaap:Cash"].namespace = ""
  subject.concepts["us-gaap:Assets"].references = [
    Reference(value="Disclosure 1", role="http://example.com/ref")
  ]
  subject.facts[0].raw_value = "1,000"
  subject.facts[0].source_hash = "arelle-md5"
  subject.networks[0].role_id = "BalanceSheet"
  for fact in subject.facts:
    fact.entity_scheme = subject.entity.scheme
    fact.entity_identifier = subject.entity.cik

  _, writer_gaps = to_clawdog_report(subject)
  got, reader_gaps = from_clawdog_report(to_clawdog(subject))

  assert writer_gaps.missing == []
  assert reader_gaps.to_dict() == {
    "missing": [],
    "unknown_node_types": {},
    "unsupported_equations": 0,
  }
  assert got.entity.model_dump() == subject.entity.model_dump()
  assert [p.model_dump() for p in got.periods] == [
    p.model_dump() for p in subject.periods
  ]
  assert [u.model_dump() for u in got.units] == [u.model_dump() for u in subject.units]
  assert {q: c.model_dump() for q, c in got.concepts.items()} == {
    q: c.model_dump() for q, c in subject.concepts.items()
  }
  assert sorted(str(f.model_dump()) for f in got.facts) == sorted(
    str(f.model_dump()) for f in subject.facts
  )
  assert sorted(str(n.model_dump()) for n in got.networks) == sorted(
    str(n.model_dump()) for n in subject.networks
  )
  assert got.filing.model_dump() == subject.filing.model_dump()
...
def test_clawdog_gap_report_is_empty_for_model_fields_it_writes() -> None:
  model = _clawdog_model()
  ...
  assert "fact source_hash" not in gaps.missing
  assert "fact raw_value" not in gaps.missing
  assert "network role_id" not in gaps.missing
